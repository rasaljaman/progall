...
const AdminDashboard: React.FC = () => {
  ...
  const loadImages = async () => {
    setLoading(true);
    const data = await supabaseService.getImages('all');
    setImages(data);
    setLoading(false);
  };
  ...
  const handleApprove = async (image: ImageItem) => {
    try {
      const updated = { ...image, status: 'active' as const };
      await supabaseService.updateImage(updated);
      setImages(images.map(img => img.id === image.id ? updated : img));
      ...
      <div className="max-w-6xl mx-auto">
                    ...
                    <div key={img.id} className="bg-surface border border-surfaceHighlight rounded-xl overflow-hidden group relative hover:border-accent/50 transition-colors">
                      ...
                      <div className="relative h-48 bg-black">
                        ...
                        <div className="absolute top-2 right-2 flex flex-col gap-1 items-end z-10">
                          {img.is_featured && <div className="bg-accent text-white text-[10px] font-bold px-2 py-1 rounded shadow uppercase">Featured</div>}
                          <div className={`text-[10px] font-bold px-2 py-1 rounded shadow uppercase border ${
                            img.status === 'pending'
        ...
        {activeTab === 'moderation' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold text-textPrimary">Moderation Queue</h2>
                <p className="text-textSecondary text-sm mt-1">
                  Approve, edit, or delete pending posts generated by the AI auto-pipeline.
                </p>
              </div>
            </div>
            ...
            {loading ? (
              ...
              (() => {
                ...
                if (pendingImages.length === 0) {
                  return (
                    <div className="text-center py-20 bg-surface border border-surfaceHighlight rounded-2xl shadow-neumorphic">
                      <Check size={48} className="mx-auto text-green-500 mb-4" />
                      <h3 className="text-lg font-bold text-textPrimary">All Caught Up!</h3>
                      <p className="text-textSecondary text-sm mt-1">No pending pipeline posts to moderate.</p>
                    </div>
                  );
                }
                return (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {pendingImages.map((img) => (
                      <div key={img.id} className="bg-surface border border-surfaceHighlight rounded-xl overflow-hidden group relative hover:border-accent/50 transition-colors flex flex-col justify-between animate-in fade-in duration-300">
                        <div>
                          <div className="relative h-48 bg-black">
                            <img src={img.thumbnail} alt="" className="w-full h-full object-cover opacity-80" />
                            <div className="absolute top-2 left-2 bg-yellow-500/20 text-yellow-500 text-[10px] font-bold px-2 py-1 rounded shadow uppercase border border-yellow-500/30">
                              Pending Review
                            </div>
                            {img.source && (
                          ...
                          <div className="p-4">
                            <p className="text-textPrimary text-sm line-clamp-3 mb-3">{img.prompt}</p>
                            <div className="flex flex-wrap gap-1.5 mb-4">
                              <span className="text-xs bg-surfaceHighlight px-2 py-1 rounded text-textSecondary border border-surfaceHighlight">{img.category}</span>
                              {img.tags && img.tags.slice(0, 3).map(tag => (
                                <span key={tag} className="text-[10px] bg-background px-1.5 py-0.5 rounded text-textSecondary">#{tag}</span>
                              ))}
                            </div>
                          </div>
                        </div>
                        <div className="p-4 pt-0 border-t border-surfaceHighlight/50 bg-surfaceHighlight/10 flex gap-2">
                          <button onClick={() => handleApprove(img)} className="flex-1 py-2 bg-green-600 hover:bg-green-500 text-white font-bold rounded-lg text-xs flex items-center justify-center gap-1.5 transition-colors">
                            <Check size={14} /> Approve
                          </button>
                          <button onClick={() => setEditingImage(img)} className="px-3 py-2 border border-surfaceHighlight bg-surface hover:bg-surfaceHighlight rounded-lg text-textPrimary transition-colors" title="Edit details">
                            <Edit2 size={14} />
                          </button>
                          <button onClick={() => img.id && handleDelete(img.id)} className="px-3 py-2 border border-red-500/20 text-red-500 hover:bg-red-500/10 rounded-lg transition-colors" title="Delete">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </div>
